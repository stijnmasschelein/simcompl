...
\begin{equation}\label{eq:demand1}
\delta_j x^*_{ji} = \beta_j + \gamma_j z_i + \epsilon_{ji} + \beta_{12} x^*_{ki}
\end{equation}
$$
j,k \in \{1,2\}, j \neq k
$$
...
\rho^c &= cor(x_1|z, x_2|z) =
          \frac{cor(\mathbf{x_1},\mathbf{x_2}|\mathbf{z})}
               {\sqrt{1 - cor^2(\mathbf{x_1}, \mathbf{z})}} \nonumber \\
\beta^d_{12} &= \frac{sd(\mathbf{x_1})}{sd(\mathbf{x_2)}}
                \frac{cor(\mathbf{x_1, x_2})
                      - cor(\mathbf{x_1, z}) cor(\mathbf{x_1, z})}
                {1 - cor^2(\mathbf{x_2, z})} \nonumber \\
             &= \frac{sd(\mathbf{x_1})}{sd(\mathbf{x_2)}}
                \frac{cor(\mathbf{x_1, x_2|z})}
                {\sqrt{1 - cor^2(\mathbf{x_2, z})}} \nonumber \\
             &= \frac{sd(\mathbf{x_1})}{sd(\mathbf{x_2)}}
...
\begin{figure}
```{r scatter, fig.width=6, fig.height=3, out.width="600px", warning=FALSE}
...
\begin{figure}
```{r fig.width=7, fig.height=3, out.width="600px", warning=FALSE}
...
```
\begin{figure}
```{r scatterplot-delta, fig.width=6, fig.height=3, out.width="600px", warning=FALSE}
if (runsim){
...
\begin{figure}
```{r scatterplot-delta2, fig.width=6, fig.height=3, out.width="600px", warning=FALSE}
if (runsim){
  b2 = list(c(0.5, 0, 0), c(0, 0, 0))
  N1 = length(b2)
